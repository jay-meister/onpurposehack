...
export default {
  ...
  handler: (request, reply) => {
    ...
    const oauth = {
      ...
      token_secret: process.env.ACCESS_TOKEN_SECRET, // eslint-disable-line
    }
...
const filterResponse = (twitterResponse) => {
  return twitterResponse.map(tweet => {
    ...
    : null
    const description = typeof tweet.text === 'string'
    ? tweet.text.split('@reuseapp').join('').split('https://')[0].trim()
    : null
    const imgURLs = tweet.extended_entities
    ? tweet.extended_entities.media.filter(item => item.type === 'photo')
